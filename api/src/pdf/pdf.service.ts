...
export class PdfService {
  ...
  attendanceRegister(data: any) {
    ...
    return this.render((doc: any) => {
      ...
      const dayCols = dates.slice(0, 12).map((d: string) => ({ label: d.slice(5), width: 28, align: 'center' as const }));
      this.table(
        doc,
        [
  ...
  payslip(data: { school: any; run: any; item: any }) {
    ...
    return this.render((doc: Doc) => {
      ...
      const y = doc.y;
      this.kv(doc, [['Employee', `${item.staff.firstName} ${item.staff.lastName}`], ['Employee ID', item.staff.employeeId], ['Position', item.staff.position ?? '—']], 40, y);
      this.kv(doc, [['Department', item.staff.department ?? '—'], ['Status', String(run.status)], ['Paid on', run.paidAt ? new Date(run.paidAt).toLocaleDateString('en-GB') : '—']], 320, y);
      doc.y = y + 100;
      this.table(doc, [{ label: 'Description', width: 380 }, { label: 'Amount', width: 135, align: 'right' }], [['Basic salary', `${cur} ${Number(item.basic).toFixed(2)}`], ['Allowances', `${cur} ${Number(item.allowances).toFixed(2)}`], ['Deductions', `- ${cur} ${Number(item.deductions).toFixed(2)}`]], doc.y, school.primaryColor);
      doc.font('Helvetica-Bold').fontSize(13).fillColor('#111827').text(`Net pay: ${cur} ${Number(item.net).toFixed(2)}`, 40, doc.y, { align: 'right', width: 515 });
      if (item.notes) doc.moveDown(0.5).font('Helvetica').fontSize(10).fillColor('#374151').text(`Notes: ${item.notes}`, 40, doc.y, { width: 515 });
      doc.fontSize(8).fillColor('#9ca3af').text('This payslip is confidential. Generated by School OS.', 40, doc.page.height - 50, { align: 'center', width: 515 });
    });
  }
  ...
  transcript(data: { school: any; student: any; sheets: any[] }) {
    ...
    return this.render((doc: Doc) => {
      this.header(doc, school, 'Academic Transcript', `${student.firstName} ${student.lastName} · ${student.studentId}`);
      for (const s of sheets) {
        if (doc.y > doc.page.height - 200) doc.addPage();
        doc.font('Helvetica-Bold').fontSize(11).fillColor('#111827').text(`${s.term?.academicYear?.name ?? ''} — ${s.term?.name ?? ''}   ·   Average ${Number(s.average).toFixed(1)}%  ·  Grade ${s.overallGrade ?? '—'}  ·  Position ${s.position ?? '—'} of ${s.classSize}`, 40, doc.y);
        doc.moveDown(0.3);
        const subjects: any[] = Array.isArray(s.subjects) ? s.subjects.filter((x: any) => x.hasMarks) : [];
        this.table(doc, [{ label: 'Subject', width: 235 }, { label: 'Class', width: 70, align: 'right' }, { label: 'Exam', width: 70, align: 'right' }, { label: 'Total', width: 70, align: 'right' }, { label: 'Grade', width: 70, align: 'center' }], subjects.map((x: any) => [x.name, Number(x.ca).toFixed(1), Number(x.exam).toFixed(1), Number(x.total).toFixed(1), x.grade ?? '']), doc.y, school.primaryColor);
      }
      if (!sheets.length) doc.font('Helvetica').fontSize(10).fillColor('#6b7280').text('No published results yet.', 40, doc.y);
      doc.fontSize(8).fillColor('#9ca3af').text(`Generated by School OS on ${new Date().toLocaleDateString('en-GB')}`, 40, doc.page.height - 50, { align: 'center', width: 515 });
    });
  }
  ...
  timetable(data: { school: any; title: string; periods: any[]; slots: any[] }) {
    ...
    return this.render((doc: Doc) => {
      ...
      const cell = (d: number, p: any) => {
        ...
        return s ? `${s.subject?.name ?? ''}${s.class ? ` (${s.class.name})` : ''}${s.room ? ` · ${s.room.name}` : ''}` : '';
      };
      this.table(doc, [{ label: 'Period', width: 95 }, ...days.map((d) => ({ label: d, width: 84 }))], periods.map((p) => [`${p.name}\n${p.startTime}-${p.endTime}`.replace('\n', ' '), ...days.map((_, i) => cell(i + 1, p))]), doc.y, school.primaryColor);
    });
  }
