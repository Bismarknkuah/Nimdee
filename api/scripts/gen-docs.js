...
let out =
  ...
  ' models, ' +
  dm.enums.length +
  ' enums). Every table with a `tenantId` column is protected by PostgreSQL Row-Level Security.\n\n```mermaid\nerDiagram\n';
const typeMap = {
  String: 'string',
  Int: 'int',
  Boolean: 'bool',
...
for (const m of dm.models) {
  ...
  for (const f of m.fields)
    if (f.kind !== 'object')
      out +=
        '    ' +
        (f.kind === 'enum' ? f.type : typeMap[f.type] || f.type) +
        (f.isList ? '_list' : '') +
        ' ' +
        f.name +
        (f.isId ? ' PK' : f.isUnique ? ' UK' : '') +
        '\n';
  out += '  }\n';
}
for (const m of dm.models)
...
for (const [area, names] of Object.entries(areas))
  out += '- **' + area + '**: ' + names.filter((n) => dm.models.some((m) => m.name === n)).join(', ') + '\n';
...
for (const [path, ops] of Object.entries(spec.paths))
  for (const [method, op] of Object.entries(ops)) {
    ...
    (tags[tag] ||= []).push({ method: method.toUpperCase(), path, summary: op.summary || '' });
  }
let api =
...
console.log(
  ...
  'tags,',
  Object.values(tags).reduce((a, l) => a + l.length, 0),
  'operations',
);
